...
`public/project-memory.html` はプロジェクト候補と動画の関係図、時系列、計画・実施・完成・問題の原文を表示します。検索は収集済み字幕の全文とタイトル・概要欄を対象にします。Jevの関連判定と出来事判定を**同じ区間**で対応付けるため、話題が同じだけの別件を自動的に「解決済み」と扱いません。計画は続報の確認候補として表示します。
